...
// Level 1
    ...
    LL_findAll_test(num);
// Level 2
    LL_pushMany_test(num);
    LL_popMany_test(num);
    LL_freeMany_test(num);
    LL_insertAfter_test(num);
    LL_insertAtIndex_test(num);
// Level 3
    LL_forEach_test(num);
...
void LL_find_test(int count) {
    nodePtr head = newTestLL(count), n;
    int len = LL_length(head) / 2;
    nodePtr indexedN = LL_ofIndex(head, len),
            foundN = LL_find(head, indexedN->value);
    processTestResults("find: value in list", foundN->value == indexedN->value);
    processTestResults("find: value not in list", LL_find(head, -1) == NULL);
    LL_freeAll(&head);
}
void LL_findBefore_test(int count) {
    nodePtr head = newTestLL(count);
...
void LL_free_test(int count) {
    ...
    processTestResults("free: deallocate head", oldHead->next == head);
    processTestResults("free: deallocate elem", oldElem != head && oldElem->next == head->next);
    LL_freeAll(&head);
}
void LL_concat_test(int count) {
    nodePtr head = newTestLL(count);
...
void LL_split_test(int count) {
    ...
    head = newTestLL(17);
    LL_split(&head, &backHalf);
    processTestResults("split - odd:  [a] > [b]", LL_length(head) > LL_length(backHalf));
    LL_freeAll(&head);
    LL_freeAll(&backHalf);
...
void LL_findAll_test(int count) {
    nodePtr head = newTestLL(count);
    int value = LL_ofIndex(head, count / 2)->value;
...
void LL_popMany_test(int count) {
    nodePtr head = newTestLL(count);
    int value = count / 2;
    int* lastCountValues = LL_toArray(LL_lastN(head, value));
    int* poppedCountValues = LL_popMany(&head, value);

    int results, i;
    for (results = 1, i = 0; i < value; i++)
...
void LL_freeMany_test(int count) {
    nodePtr head = newTestLL(count);
    LL_freeMany(&head, &head->next, count);
    processTestResults("freeMany", head && !head->next);
    LL_freeAll(&head);
}
void LL_insertAfter_test(int count) {
    nodePtr head = newTestLL(count);
...
void LL_insertAtIndex_test(int count) {
    ...
    int valueToFind = head->next->next->value; // A value several nodes into the list.
...
void LL_forEach_test(int count) {
    nodePtr head = newTestLL(count),
            n = head;
    int i, len = LL_length(head);
    testF_Values = malloc(len * sizeof(int));
